...
#else
# define readline(p) getline_sqlite_internal(p,stdin)
...
** is done.
*/
static char *getline_sqlite_internal(char *zPrompt, FILE *in){
  char *zLine;
  int nLine;
  int n;
  int eol;
...
** is false, use "getline_sqlite_internal" instead of "readline" and issue no prompt.
**
...
static char *one_input_line(const char *zPrior, FILE *in){
  ...
  char *zResult;
  if( in!=0 ){
    return getline_sqlite_internal(0, in);
  }
  if( zPrior && zPrior[0] ){
